...
const CharacterPage: React.FC<CharacterPageProps> = () => {
    ...
    const isInFavorites = favorites.some((item: Character) => item.id === character?.id);
    ...
    if (isError) return <div className="text-center text-red-500 text-xl mt-10">Error loading character...</div>;
    if (!character) return <div className="text-center text-white text-xl mt-10">Character not found or data missing...</div>;
    ...
    return (
        <div className="container mx-auto p-4 md:p-8">
            ...
            >   
                <span className="p-2 rounded-full bg-orange-400"><FaArrowLeft /></span>
                    ...
                    {character.episode.length > 0 && (
                        <div className="mt-6">
                            ...
                            <ul className="grid grid-cols-1 gap-2 max-h-300 overflow-y-auto pr-2 custom-scrollbar">
                                {(showAllEpisodes ? character.episode : character.episode.slice(0, 5)).map((episodeUrl: string, index: number) => (
                                        <EpisodeInfo
                                            key={index}
